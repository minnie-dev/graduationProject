...
public class test extends AppCompatActivity implements TMapGpsManager.onLocationChangedCallback {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState){
        ...
        Intent intent = getIntent(); // MainActivity에서 전달한 값 받기
        String ExcelLat = intent.getStringExtra("ExcelLat");
        String ExcelLon = intent.getStringExtra("ExcelLon");
        String Name = intent.getStringExtra("name");
        double Lat = Double.valueOf(ExcelLat).doubleValue(); // 전달받은 ExcelLat값 double형으로 변환
        double Lon = Double.valueOf(ExcelLon).doubleValue(); // 전달받은 ExcelLon값 double형으로 변환
        ...
        tMapView.setCenterPoint(Lon,Lat); // 화면 중심을 목적지로 이동
...
//        dialog();
            ...
            public void onCalloutRightButton(final TMapMarkerItem tMapMarkerItem) {
                ...
                builder.setPositiveButton("시작", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        Toast.makeText(test.this,"목적지 설정완료!!",Toast.LENGTH_SHORT).show();
                        ...
                        tmapdata.findPathDataWithType(TMapData.TMapPathType.PEDESTRIAN_PATH, tMapPointStart, tMapPointEnd, new TMapData.FindPathDataListenerCallback() {
                            ...
                            public void onFindPathData(TMapPolyLine polyLine) {
                                System.out.println("폴리라인 시작점 확인 " + tMapPointStart);
                        ...
                        System.out.println("@@@@@@@@@@@@@@ url결과 확인 : " + urlSave);
        ...
        bt.setOnDataReceivedListener(new BluetoothSPP.OnDataReceivedListener() { //데이터 수신
            public void onDataReceived(byte[] data, String message) {
                Toast.makeText(test.this, message, Toast.LENGTH_SHORT).show();  //확인용
                if(message.equals("1")) {
                    checkNum +=1;
        ...
        bikeimg.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                exceltest();
                Toast.makeText(test.this,"반경 5km 검색결과입니다.",Toast.LENGTH_SHORT).show();
